...
useEffect(() => {
  console.log("Updated typingUser:", typingUser);
}, [typingUser]);
...
const typingTimer = useRef(null);
  ...
  if (stompClient && stompClient.connected) {
    ...
    stompClient.send(`/app/typing/${roomId}`, {}, JSON.stringify(typingPayload));
  }
};
  ...
  return (
    <div>
      {/* Header */}
      <header className="flex justify-around items-center py-5 fixed w-full z-10 
        ...
        <div>
          <button onClick={handleLogout} className="bg-red-500 hover:bg-red-700 text-white px-3 py-2 rounded-full">
            Leave Room
          </button>
        </div>
      </header>
      ...
      <main ref={chatBoxRef} className="pt-20 px-6 w-[60%] bg-gray-300 shadownpm install tailwind-scrollbar-hide
 dark:bg-slate-800 mx-auto h-[calc(100vh-6rem)] overflow-y-scroll scrollbar-hide">
          {messages.map((message, index) => (
          <div key={index} className={` flex ${message.sender === currentUser ? "justify-end" : "justify-start"}`}>
            <div className={`p-3 my-2 max-w-xs rounded-2xl shadow-sm border
              ${message.sender === currentUser
                ? "bg-green-500 text-white"
                : "bg-gray-200 text-gray-900 border-gray-300 dark:bg-white dark:text-black"}
              dark:border-black`}>
              <div className="flex flex-row items-center">
                <img className="h-10 w-10 rounded-full" src={"https://avatar.iran.liara.run/public/27"} alt="" />
                <div className="flex flex-col pl-2">
                  <p className="text-sm font-bold">{message.sender}</p>
                  <p>{message.content}</p>
                  <p className="text-xs text-gray-600 dark:text-gray-800">{timeAgo(message.timestamp)}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="bottom-2 fixed w-full h-16">
        <div className="bg-gray-100 text-black border border-gray-300 rounded-full pr-8 
          ...
          <div className="flex gap-2">
            ...
            <button onClick={sendMessage} className="bg-green-600 h-10 w-10 flex justify-center items-center rounded-full text-white">
              <MdSend size={20} />
            </button>
          </div>
        </div>
